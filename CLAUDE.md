...
| `evidence.ts` | `toEvidenceBundle(receipts, { signer })` — serialize InclusionReceipts → ONE signed, portable `ario.evidence/v1` / `ario.anchor.trace/v1` bundle (emit side only; **verify + the `npx @ar.io/proof verify` CLI live in `@ar.io/proof`**, never here — auditors must not need the write SDK). Body = the receipts' envelopes + checkpoints + inclusion proofs; wrapper signed with the anchorer's own key. Implemented on `main`; live once both publish. |
| `anchorer.ts` | `createAnchorer()` — the structural dev/prod gate — and the single upload pipeline. The returned `Anchorer` also exposes `bundle(receipts, opts?)`: zero-ceremony `toEvidenceBundle` wired to the anchorer's OWN signer (no second key to hand around). |
